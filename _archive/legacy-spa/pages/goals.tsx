...
import type { Goals, GoalStatus, GoalType } from "../helpers/schema";
...
export default function GoalsPage() {
  const [goalType, setGoalType] = useState<GoalType | "_all">("_all");
  const [status, setStatus] = useState<GoalStatus | "_all">("_all");
  
  const handleGoalTypeChange = (value: string) => {
    setGoalType(value as GoalType | "_all");
  };

  const handleStatusChange = (value: string) => {
    setStatus(value as GoalStatus | "_all");
  };

  const { data, isLoading } = useGoalsList({
    goalType: goalType === "_all" ? undefined : goalType,
