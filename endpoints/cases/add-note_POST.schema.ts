...
export async function postAddCaseNote(body: InputType, init?: RequestInit): Promise<OutputType> {
  const result = await fetch(`/_api/cases/add-note`, {
    method: "POST",
  ...
  if (!result.ok) {
    ...
    throw new Error((typeof parsed?.error === "string" ? parsed.error : parsed?.error?.message) ?? "Failed to add case note");
  }
