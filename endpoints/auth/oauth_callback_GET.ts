...
export async function handle(request: Request) {
  try {
    ...
    if (existingUsers.length > 0) {
      ...
      // Create workspace for new user (hybrid identity model: 1 user = 1 workspace)
      const newWorkspaces = await db
        .insertInto("workspaces")
        .values({
          name: `${mappedUserData.displayName}'s Workspace`,
          createdAt: now,
          updatedAt: now,
        })
        .returning(["id"])
      ...
      // Create workspace member record linking user to workspace as owner
      await db
