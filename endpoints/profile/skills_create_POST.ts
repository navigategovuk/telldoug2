...
import { createSchema, CreateOutputType } from "./skills.schema";
...
export async function handle(request: Request) {
  try {
    ...
    await db.insertInto("skills").values({
      ...
      name: input.name, level: input.level, keywords: input.keywords ?? null,
      proficiency: input.proficiency, category: input.category, notes: input.notes,
      sortOrder: input.sortOrder,
    }).execute();

    const skill = await db.selectFrom("skills").selectAll().where("id", "=", id).executeTakeFirstOrThrow();
    return new Response(superjson.stringify({ skill, created: true } satisfies CreateOutputType));
  } catch (error) {
